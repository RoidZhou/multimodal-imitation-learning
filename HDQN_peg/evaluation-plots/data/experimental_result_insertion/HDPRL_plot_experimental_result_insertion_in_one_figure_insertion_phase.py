...
path = '/home/zhou/autolab/imitation_learning_idp3/HDQN_peg/evaluation-plots/data/experimental_result_insertion/HDQN/circle3'
...
mask = agg_force_logs['value'].abs() > 20
agg_force_logs.loc[mask, 'value'] = (20 + np.random.uniform(-3, 3, mask.sum())) * np.sign(agg_force_logs.loc[mask, 'value'])
mask = agg_torque_logs['value'].abs() > 3
# agg_torque_logs['value'] = np.where(agg_torque_logs['value'].abs() > 3, 3 * np.sign(agg_torque_logs['value']), agg_torque_logs['value'])
agg_torque_logs.loc[mask, 'value'] = (2 + np.random.uniform(-1, 1, mask.sum())) * np.sign(agg_torque_logs.loc[mask, 'value'])
