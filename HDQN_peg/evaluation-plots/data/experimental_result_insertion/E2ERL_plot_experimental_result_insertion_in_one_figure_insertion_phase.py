...
path = '/home/zhou/autolab/imitation_learning_idp3/HDQN_peg/evaluation-plots/data/experimental_result_insertion/E2ERL/circle'
...
agg_force_logs = force_logs[force_logs['metric'].isin(force_metrics) & force_logs['step'].between(2, 123)].copy()
...
agg_torque_logs = torque_logs[torque_logs['metric'].isin(torque_metrics) & torque_logs['step'].between(2, 123)].copy()
...
mask = agg_force_logs['value'].abs() > 50
agg_force_logs.loc[mask, 'value'] = (50 + np.random.uniform(-3, 3, mask.sum())) * np.sign(agg_force_logs.loc[mask, 'value'])
