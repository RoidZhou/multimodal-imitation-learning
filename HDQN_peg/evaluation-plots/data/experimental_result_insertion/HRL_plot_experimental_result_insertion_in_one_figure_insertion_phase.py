...
square = square
"""
path = '/home/zhou/autolab/imitation_learning_idp3/HDQN_peg/evaluation-plots/data/experimental_result_insertion/HRL/square'
...
# Filter specific metrics and steps (0 to 100)
force_metrics = ['force_x', 'force_y', 'force_z']
torque_metrics = ['Torque_x', 'Torque_y', 'Torque_z']
agg_force_logs = force_logs[force_logs['metric'].isin(force_metrics) & force_logs['step'].between(0, 40)].copy()
agg_force_logs['display_step'] = (agg_force_logs['step'] - 0).astype(np.int64)  # Map step 0-100 to 0-100
agg_force_logs = agg_force_logs.reset_index(drop=True)
agg_torque_logs = torque_logs[torque_logs['metric'].isin(torque_metrics) & torque_logs['step'].between(0, 40)].copy()
agg_torque_logs['display_step'] = (agg_torque_logs['step'] - 0).astype(np.int64)  # Map step 0-100 to 0-100
agg_torque_logs = agg_torque_logs.reset_index(drop=True)

''' Append 5 zeros for each label '''
# new_steps = np.arange(41, 46)  # Steps 41 to 45
# append_force_logs = pd.DataFrame()
# append_torque_logs = pd.DataFrame()
#
# for idx, label in enumerate(force_custom_labels):
#     new_data = pd.DataFrame({
#         'metric': [force_metrics[idx]] * 5,
#         'value': [0.0] * 5,
#         'step': new_steps,
#         'label': [label] * 5
#     })
#     append_force_logs = pd.concat([append_force_logs, new_data], ignore_index=True)
#
# for idx, label in enumerate(torque_custom_labels):
#     new_data = pd.DataFrame({
#         'metric': [torque_metrics[idx]] * 5,
#         'value': [0.0] * 5,
#         'step': new_steps,
#         'label': [label] * 5
#     })
#     append_torque_logs = pd.concat([append_torque_logs, new_data], ignore_index=True)
#
# # Concatenate original and appended data
# agg_force_logs = pd.concat([agg_force_logs, append_force_logs], ignore_index=True)
# agg_torque_logs = pd.concat([agg_torque_logs, append_torque_logs], ignore_index=True)
#
# # Update display_step
# agg_force_logs['display_step'] = (agg_force_logs['step'] - 0).astype(np.int64)  # Map step 0-45 to 0-45
# agg_torque_logs['display_step'] = (agg_torque_logs['step'] - 0).astype(np.int64)  # Map step 0-45 to 0-45
# agg_force_logs = agg_force_logs.reset_index(drop=True)
# agg_torque_logs = agg_torque_logs.reset_index(drop=True)
""" Append 5 zeros for each label """


# Apply value filtering: set |value| > 50 to 20 (preserve sign)
agg_force_logs['value'] = np.where(agg_force_logs['value'].abs() > 50, 20 * np.sign(agg_force_logs['value']), agg_force_logs['value'])
agg_torque_logs['value'] = np.where(agg_torque_logs['value'].abs() > 3, 3 * np.sign(agg_torque_logs['value']), agg_torque_logs['value'])

# Debug: Check filtered data and value ranges
...
print(f"agg_force_logs value range: {agg_force_logs['value'].min():.2f} to {agg_force_logs['value'].max():.2f}")
print(f"agg_torque_logs value range: {agg_torque_logs['value'].min():.2f} to {agg_torque_logs['value'].max():.2f}")
...
sns.set_context("notebook", font_scale=1.4)  # Restore font scale from previous context
