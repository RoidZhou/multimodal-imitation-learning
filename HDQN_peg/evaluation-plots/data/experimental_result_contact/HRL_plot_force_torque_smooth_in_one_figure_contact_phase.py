...
agg_force_logs = force_logs[force_logs['metric'].isin(force_metrics) & force_logs['step'].between(1, 170)].copy()
agg_force_logs['display_step'] = (agg_force_logs['step'] - 1).astype(np.int64)  # Map step 10-200 to 0-190
agg_force_logs = agg_force_logs.reset_index(drop=True)
agg_torque_logs = torque_logs[torque_logs['metric'].isin(torque_metrics) & torque_logs['step'].between(1, 170)].copy()
agg_torque_logs['display_step'] = (agg_torque_logs['step'] - 1).astype(np.int64)  # Map step 10-200 to 0-190
...
# axes[0].legend(loc='upper right', fancybox=True, bbox_to_anchor=(0.00, 1.02), shadow=False, framealpha=0.6, ncol=1, prop={'size': 14})
axes[0].set_xlabel('')
...
# axes[1].legend(loc='upper right', fancybox=True, bbox_to_anchor=(0.0, 1.02), shadow=False, framealpha=0.6, ncol=1, prop={'size': 14})
