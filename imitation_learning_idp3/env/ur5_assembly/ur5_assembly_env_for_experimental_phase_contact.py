...
class UR5Env:
    ...
    def __init__(self, cfg, render=True):
        ...
        self.writer_FT = SummaryWriter('./HDQN_peg/collect_dataset_log/phase_contact_FT')
        ...
        self.set_link_stiffness(self.ur5_id, self.ur5EndEffectorIndex, contact_stiffness, contact_damping, self.physicsClient_use)
        self.numdof = 6
    ...
    def run(self):
        ...
        time1 = 3.5
        end_peg_matrix0_1 = end_peg_matrix
        end_peg_matrix0_1[2, 3] -= 0.0015
        T0_1 = SE3(end_peg_matrix0_1)
        t2 = T0_1.t
        ...
        actions = np.zeros((every_epoch_num, self.action_dim+3))
        ...
        forces = np.zeros((every_epoch_num, self.force_dim+3))
        ...
        while p.isConnected():
            ...
            if time_num % every_step_num == 0:
                ...
                actions[data_num, 0:3] = peg_position
                actions[data_num, 3:self.action_dim+3] = peg_orientation

                images[data_num, ...] = image
                images_hand[data_num, ...] = image_hand
                depths[data_num, ...] = depth
                forces[data_num, ...] = force
                data_num += 1
            ...
            """plot contact phase force/torque"""
            # 获取当前的关节力和力矩
            FT = self.getForceTorque()
            Fext = [FT[0], FT[1], FT[2]]
            Text = [FT[3], FT[4], FT[5]]
            ...
            self.writer_FT.add_scalars("force_x",
                                    {"force_x": self.force_x}, time_num)
            self.writer_FT.add_scalars("force_y",
                                    {"force_y": self.force_y}, time_num)
            self.writer_FT.add_scalars("force_z",
                                    {"force_z": self.force_z}, time_num)
            self.writer_FT.add_scalars("Torque_x",
                                    {"Torque_x": self.Torque_x}, time_num)
            self.writer_FT.add_scalars("Torque_y",
                                    {"Torque_y": self.Torque_y}, time_num)
            self.writer_FT.add_scalars("Torque_z",
                                    {"Torque_z": self.Torque_z}, time_num)
            """plot contact phase force/torque"""
    ...
    def get_observation(self):
        ...
        force = np.array(self.getForceTorque()[:6])
    ...
    def impedance_controller(self, action, physicsClientId, if_test):
        ...
        for i in range(1):
            ...
            if if_test == True:
                ...
                quat_rot_err_tmp = quat_rot_err_ * 1000
    ...
    def test_impedance_controller(self, action, physicsClientId, if_test):
        """
        z方向使用力控制
        x,y方向使用导纳控制
        input:
        ...
        force_z_adjustment = force_control_output_z
        pos_z_adjustment = max(min(force_z_adjustment, delta_z_max), -delta_z_max)
        pos_z_adjustment = pos_z_adjustment / 2  # 手动补偿
        if wrench_z > 0.1:
            # print("wrench_z : ", wrench_z)
            pos_z_adjustment = pos_z_adjustment
        for i in range(1):
    ...
    def apply_hybrid_controller(self, action, physicsClientId, if_test=False, flag=1):
        ...
        self.translational_damping = 280.28
        self.translational_damping = 280.28
        self.duration = 0.002
        ...
        while 1:
            if if_test:
                desired_twist, deta_desired_twist = self.test_impedance_controller(action, physicsClientId, if_test)
            else:
                desired_twist, deta_desired_twist = self.impedance_controller(action, physicsClientId, if_test)
            ...
            if if_test == True:
                if flag==0:
                    if (abs(self.rz) < 5e-4 and abs(
                            self.force_z) > 0.01) or self.insert_depth > 0.436 or self.solve_steps > 5000:
                        # if (abs(self.rz) < 5e-6):
                        print("force err success")
                        break
                if (abs(self.rz) < 5e-4 and abs(self.deta_dz) < 5e-8) or self.insert_depth > 0.436 or self.solve_steps > 5000:
                # if (abs(self.rz) < 5e-6):
                    print("force err success")
                    break
            else:
    ...
    def send_commands_to_robot(self, vx, vy, vz, wx, wy, wz, physicsClientId):
        ...
        for i, joint_index in enumerate(joint_indices):
            if joint_index == 6:
                ...
                # ----------------------将更新频率设置为真实频率---------------------
        p.setTimeStep(1.0 / self._timeStep)
        for _ in range(4): # E2ERL: triangle=4, square=2
            p.stepSimulation()