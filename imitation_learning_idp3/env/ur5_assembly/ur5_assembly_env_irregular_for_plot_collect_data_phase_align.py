...
class UR5Env:
    ...
    def __init__(self, cfg, render=True):
        ...
        self.ur5_id = p.loadURDF(
            "./assert/ur_description/urdf/ur5_robot_sensor_eelink_irregular.urdf",
    ...
    def test_hybrid_controller(self):
        ...
        # self.reset()
        angles = []
        angles_select = [-90, -45, 0, 45, 90]
        for i in range(5):
            hole_orientation = Rotation.from_euler('xyz', [90, 90, -90+45*i], degrees=True).as_quat()  # 默认固定孔的姿态
    ...
    def reset(self):
        ...
        # self.init_joint_val[4] += init_joint0_orien
        ...
        self.target_joint_angles = p.calculateInverseKinematics(
            ...
            # targetOrientation=hole_orientation,
        ...
        self.prev_pos = np.array(current_peg_pos)
        self.prev_pos[2] -= 0.025
    ...
    def run(self):
        ...
        angles = []
        angles_select = [-90, 90, 270]
        for i in range(5):
            hole_orientation = Rotation.from_euler('xyz', [90, 90, -90+180*i], degrees=True).as_quat()  # 默认固定孔的姿态
        ...
        time3 = 0.5
        t3 = t2.copy()
        t3[2] -= 0.003 # for plot collect data phase contact
        R3 = R2.copy()
