...
class UR5Env(gym.Env):
    ...
    def __init__(self, cfg, agent, render=True):
        super().__init__()
        self.log =[]
        self.horizon = 3
        ...
        self.ur5_id = p.loadURDF(
            ...
            basePosition=[0, 0, 0.1], flags=9, physicsClientId = self.physicsClient_use)
        ...
        # 设定参数动作空间
        self.observation_space = spaces.Box(low=-1.0, high=1.0, shape=(1027,), dtype=np.float32)
        ...
        #     dict(
        #         image=gym.spaces.Box(0, 255, shape=(self.image_height, self.image_width, 3), dtype=np.uint8),
        #         image_hand=gym.spaces.Box(0, 255, shape=(self.image_height, self.image_width, 3), dtype=np.uint8),
        #         agent_pos=gym.spaces.Box(-1, 1, shape=(7, ), dtype=np.float32),
        #         force=gym.spaces.Box(-100, 100, shape=(3, ), dtype=np.float32),
        #     )
        # )
    ...
    def reset(self):
        ...
        self.impedance_arrive = 0
        self.mointor_force_torque = np.zeros((2, 60))
        p.enableJointForceTorqueSensor(self.ur5_id, 7, physicsClientId=self.physicsClient_use)
        p.stepSimulation()
        ...
        for i in range(6):
            p.resetJointState(bodyUniqueId=self.ur5_id, jointIndex=i + 1, targetValue=self.init_joint_val[i], physicsClientId=self.physicsClient_use)
        ...
        try:
            ...
            print(f"IK failed: {e}, Target: {self.hole_up_end}, Client: {self.physicsClient_use}")
            raise
        for i in range(6):
            p.resetJointState(bodyUniqueId=self.ur5_id, jointIndex=i + 1, targetValue=self.target_joint_angles[i], physicsClientId=self.physicsClient_use)
        ...
        # 定义平移
        current_pos = p.getLinkState(self.ur5_id, 7, physicsClientId=self.physicsClient_use)[4]
        current_orie = p.getLinkState(self.ur5_id, 7, physicsClientId=self.physicsClient_use)[5]
        #打印出效果用
        ...
        robot_current_state = p.getLinkState(self.ur5_id, self.ur5EndEffectorIndex, computeForwardKinematics=1, physicsClientId=self.physicsClient_use)
        ...
        # 计算当前深度
        po = p.getLinkState(self.ur5_id, 7, physicsClientId=self.physicsClient_use)
    ...
    def step(self, action):
        episode_reward = 0
        self.goal = action
        print("action_code", action)

        while True:
            obs = self.get_raw_observation()
            action_code = self.agent.select_action(self.agent.meta_policy, self.robot_current_position,
                                                   self.robot_current_orientation, obs, action)
            last_goal = action
            self.step_counter += 1
            ...
            if action_code is not None:
                ...
                robot_prev_state = p.getLinkState(self.ur5_id, self.ur5EndEffectorIndex, computeForwardKinematics=1, physicsClientId=self.physicsClient_use)
                ...
                action_position = np.array(action_code[0:3])
                action_orientation_6d = np.array(action_code[3:self.action_dim])
                ...
                # 计算当前状态
                robot_state = p.getLinkState(self.ur5_id, self.ur5EndEffectorIndex, computeForwardKinematics=1, physicsClientId=self.physicsClient_use)
            ...
            for i in range(4):
                ...
                robot_state_position = p.getLinkState(self.ur5_id, self.ur5EndEffectorIndex, computeForwardKinematics=1, physicsClientId=self.physicsClient_use)
            ...
            if self.goal == 1:
                ...
                if insert_depth > 0.004 and self.goal_1_reach == 0:
                    # done = True
                    reward = 3
                    self.success_res = True
                    self.goal_1_reach = 1
                if insert_depth > 0.007:
                    done = True
                    reward = 5
                    print("success insert")
                if (abs(current_force[0]) > 20 or abs(current_force[1]) > 20 or abs(current_force[2]) > 20):
                    reward = -0
                    done = True
            ...
            info = {"is_success": np.array(self.success_res == True, dtype=np.float64)}
            ...
            if done:
                Done = True
                break
            if action == 0 and self.goal_0_reach == 1:
                break
            if action == 1 and self.goal_1_reach == 1:
                break
            if action == 2 and (self.goal_0_reach == 0 or self.goal_1_reach == 0):
                break

        return observation, episode_reward, done, info

    def seed(self, seed=None): # 新版 Gym 推荐用 reset(seed=seed)，但这里为了兼容旧 API 保留此方法
    ...
    def getForceTorque(self):
        ...
        """
        FT = []
        force = np.array(p.getJointState(self.ur5_id, 7, physicsClientId=self.physicsClient_use)[2][0:3], dtype=float).reshape(3, 1)
        torque = np.array(p.getJointState(self.ur5_id, 7, physicsClientId=self.physicsClient_use)[2][3:6], dtype=float).reshape(3, 1)
    ...
    def get_observation(self):
        ...
        # 转换为编码器需要的格式
        encoder_input = self.agent.prepare_eval_observation(observation)
        encoder_batch = dict_apply(encoder_input, lambda x: x.to(self.agent.device, non_blocking=True) if isinstance(x, torch.Tensor) else x)
        with torch.no_grad():  # 确保不计算梯度（推理模式）
            nobs_features = self.agent.obs_encoder(encoder_batch)
            return nobs_features.cpu()  # 直接使用 tensor


    def get_raw_observation(self)  -> Dict[str, np.ndarray]:
        self.camera_Position = p.getLinkState(self.ur5_id, 8, computeForwardKinematics=1, physicsClientId=self.physicsClient_use)[0]
        self.camera_Orientation = p.getLinkState(self.ur5_id, 8, computeForwardKinematics=1, physicsClientId=self.physicsClient_use)[1]
        ...
        robot_position = p.getLinkState(self.ur5_id, self.ur5EndEffectorIndex, computeForwardKinematics=1, physicsClientId=self.physicsClient_use)
    ...
    def distance_to_goal_rotation(self):
        ...
        robot_orientation = np.array(p.getLinkState(self.ur5_id, self.ur5EndEffectorIndex, physicsClientId=self.physicsClient_use)[5])
    ...
    def send_commands_to_robot(self, vx, vy, vz, wx, wy, wz, physicsClientId):
        ...
        end_pos, end_ori, com_trn, com_rot, frame_pos, frame_rot, link_vt, link_ve = p.getLinkState(self.ur5_id,
                                                                                                    ...
                                                                                                    physicsClientId=physicsClientId)
        joint_indices = []
        num_joints = p.getNumJoints(self.ur5_id, physicsClientId=physicsClientId)
        for i in range(num_joints):
            joint_info = p.getJointInfo(self.ur5_id, i, physicsClientId=physicsClientId)
