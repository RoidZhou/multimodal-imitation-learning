...
class SimpleIDP3Workspace(iDP3Workspace):
            ...
            for local_epoch_idx in tqdm.tqdm(range(cfg.training.num_epochs), desc=f"Training"):
                ...
                if (self.epoch % cfg.training.val_every) == 0:
                    with torch.no_grad():
                        ...
                        for batch_idx, batch in enumerate(train_dataloader):
                            batch = dict_apply(batch, lambda x: x.to(device, non_blocking=True) if isinstance(x,
                                                                                                              torch.Tensor) else x)
                            obs_dict = batch['obs']
                ...
                if (self.epoch % cfg.training.checkpoint_every) == 0 and cfg.checkpoint.save_ckpt:
                    ...
                    torch.save(policy.state_dict(), f"./model_{self.epoch}.pth")
